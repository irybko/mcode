...
//
func (dcpntr *DirCache) SetFields(dirpath string, portnum int) {
	//
	...
	for index < maxlen {
		...
		dcpntr.cachefiles[index].respinst.Header().Set("Content-Length", fmt.Sprintf("%d",GetFSize(curpath)))
		dcpntr.cachefiles[index].respinst.Header().Set("Content-Type",   GetContentType(curpath))
...
func (dcpntr *DirCache) RunFileServer(netaddr string) {
	//
	_, err1 := regexp.Match(`^:[a-z]*$`, []byte(fmt.Sprintf("%d", dcpntr.portnum))) 
	if err1 != nil {
	...
	for index < mlen {
		...
		path 	:= fmt.Sprintf("%s/%s", dcpntr.dirpath,dcpntr.cachefiles[index].filename) 
 		_, err2 := regexp.Match(`^(/[a-z][A-Z]*/)*$`, []byte(path))
		if err2 != nil {
		...
		go func() {
			...
			fs := http.HandlerFunc(func(resp http.ResponseWriter, req *http.Request) {
				//
				resp = dcpntr.cachefiles[index].respinst
